...
export default function ServicesClient({ userId, services: initial, metiers }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setForm({ titre: "", description: "", prix: "", duree_minutes: "", metier: "", tags: [] });
    setShowForm(false);
    setIsSubmitting(false);
    router.refresh();
  }
        ...
        <div className="bg-white border border-gray-200 rounded-2xl p-6 mb-6 shadow-sm">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <select
                value={form.metier}
                onChange={(e) => setForm({ ...form, metier: e.target.value, tags: [] })}
                className="w-full rounded-lg border border-gray-300 px-3.5 py-2.5 text-sm focus:border-brand-500 focus:outline-none focus:ring-2 focus:ring-brand-500/20"
              >
            ...
            <div key={service.id} className="bg-white border border-gray-200 rounded-xl px-5 py-4">
              {editingId === service.id ? (
                <div className="space-y-3">
                  ...
                  <select value={editForm.metier} onChange={e => setEditForm(f => ({...f, metier: e.target.value, tags: []}))}
                    className="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm">
                    <option value="">Sélectionner un métier</option>
                ...
                <div className="flex items-center justify-between gap-4">
                  ...
                  <div className="flex items-center gap-1.5 flex-shrink-0">
                    <button onClick={() => startEdit(service)}
                      className="p-2 lg:px-3 lg:py-1.5 rounded-lg border border-brand-200 text-brand-600 hover:bg-brand-50 transition-colors"
                      title="Modifier">
                      ...
                      <span className="hidden lg:inline text-xs">Modifier</span>
                    </button>
                    <button onClick={() => handleToggle(service)}
                      className="p-2 lg:px-3 lg:py-1.5 rounded-lg border border-gray-200 text-gray-600 hover:bg-gray-50 transition-colors"
                      ...
                      <span className="hidden lg:inline text-xs">{service.actif ? "Désactiver" : "Activer"}</span>
                    </button>
                    <button onClick={() => handleDelete(service.id)}
                      className="p-2 lg:px-3 lg:py-1.5 rounded-lg border border-red-200 text-red-600 hover:bg-red-50 transition-colors"
                      title="Supprimer">
                      <svg className="w-4 h-4 lg:hidden" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
