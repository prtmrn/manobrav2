...
export default async function RecherchePage({ searchParams }: PageProps) {
  ...
  filtered.sort((a, b) => {
    // Inscrits toujours avant vitrines
    const typeA = a.type === "vitrine" ? 1 : 0;
    const typeB = b.type === "vitrine" ? 1 : 0;
      ...
      <nav className="sticky top-0 z-20 bg-white/95 backdrop-blur-sm border-b border-gray-100 h-14">
        <div className="max-w-7xl mx-auto px-4 h-full flex items-center justify-between">
          <Link href="/" className="flex items-center">
            <Image src="/logo-manobra.png" alt="Manobra" width={315} height={128} className="h-8 w-auto" priority />
          </Link>
        ...
        {vue === "carte" ? (
          ...
          <>
            ...
            {paginated.length === 0 ? (
              ...
              <>
                {/* Artisans qui proposent le service */}
                {paginated.filter(p => p.serviceMatch).length > 0 && (
                  <>
                    <p className="text-sm font-semibold text-gray-700 mb-3">
                      {paginated.filter(p => p.serviceMatch).length} artisan{paginated.filter(p => p.serviceMatch).length > 1 ? "s" : ""} proposent ce service
                    </p>
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 mb-8">
                      {paginated.filter(p => p.serviceMatch).map((p, cardIndex) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <div className="relative h-40 bg-gray-100 overflow-hidden">
                              {p.photo_url ? (
                                <Image src={p.photo_url} alt={`${p.prenom ?? ""} ${p.nom ?? ""}`} fill sizes="(max-width: 640px) 100vw, 25vw" className="object-cover group-hover:scale-105 transition-transform duration-300" />
                              ) : (
                              ...
                              {p.plan_actif && p.plan_actif !== "aucun" && <span className="absolute top-2 right-2 bg-amber-400 text-amber-900 text-[10px] font-bold px-2 py-0.5 rounded-full shadow-sm">PRO</span>}
                              {p.note_moyenne > 0 && (
                                <div className="absolute bottom-2 left-2 flex items-center gap-1 bg-black/50 backdrop-blur-sm rounded-full px-2 py-0.5">
                                  ...
                                  <span className="text-white text-xs font-semibold">{p.note_moyenne.toFixed(1)}</span>
                                  {p.nombre_avis > 0 && <span className="text-white/70 text-[10px]">({p.nombre_avis})</span>}
                                </div>
                              )}
                            </div>
                            <div className="p-4">
                              <p className="font-bold text-gray-900 text-sm leading-tight truncate mb-0.5">{p.prenom} {p.nom}</p>
                              <p className="text-xs font-medium mb-2" style={{ color: config.color }}>{Array.isArray(p.metier) ? p.metier.join(" · ") : p.metier}</p>
                              {p.nombre_avis > 0 ? (
                                <div className="flex items-center gap-1 mb-2">
                                  <div className="flex items-center gap-0.5">
                                    {[1,2,3,4,5].map(i => <svg key={i} className={`w-3.5 h-3.5 flex-shrink-0 ${i <= Math.round(p.note_moyenne) ? "text-yellow-400" : "text-gray-200"}`} fill="currentColor" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" /></svg>)}
                                  </div>
                                  <span className="text-xs text-gray-500 ml-0.5">{p.note_moyenne.toFixed(1)} ({p.nombre_avis} avis)</span>
                                </div>
                              ) : <p className="text-xs text-gray-400 mb-2">Nouveau</p>}
                              <div className="flex items-center justify-between gap-2 text-xs text-gray-400 mt-auto">
                                <span className="flex items-center gap-1 truncate"><svg className="w-3 h-3 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M5.05 4.05a7 7 0 119.9 9.9L10 18.9l-4.95-4.95a7 7 0 010-9.9zM10 11a2 2 0 100-4 2 2 0 000 4z" clipRule="evenodd" /></svg><span className="truncate">{p.ville}</span></span>
                                {p.prixMin !== null && <span className="font-semibold text-gray-600 flex-shrink-0">dès {new Intl.NumberFormat("fr-FR", { style: "currency", currency: "EUR", maximumFractionDigits: 0 }).format(p.prixMin)}</span>}
                              </div>
                            </div>
                ...
                {paginated.filter(p => !p.serviceMatch).length > 0 && (
                  <>
                    <div className="flex items-center gap-3 mb-4">
                      ...
                      <p className="text-sm text-gray-400 font-medium whitespace-nowrap">Autres artisans disponibles</p>
                      <div className="flex-1 h-px bg-gray-200" />
                    </div>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="relative h-40 bg-gray-100 overflow-hidden">
                        ...
                        {/* PRO badge */}
                        {p.plan_actif && p.plan_actif !== "aucun" && (
