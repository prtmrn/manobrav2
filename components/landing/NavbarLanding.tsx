...
export default function NavbarLanding() {
  ...
  useEffect(() => {
    const check = async () => {
      ...
      setRole(r);
      if (r === "client") {
        const { data: cp } = await supabase.from("profiles_clients").select("prenom, nom").eq("id", user.id).maybeSingle();
        const p = cp as { prenom: string | null; nom: string | null } | null;
        setUserName(p?.prenom ?? user.email?.split("@")[0] ?? null);
      } else if (r === "artisan") {
        const { data: ap } = await supabase.from("profiles_artisans").select("prenom, nom").eq("id", user.id).maybeSingle();
        const p = ap as { prenom: string | null; nom: string | null } | null;
        setUserName(p?.prenom ?? user.email?.split("@")[0] ?? null);
      }
      setLoading(false);
    };
        ...
        {loading ? (
          ...
          <div className="relative" ref={dropdownRef}>
            ...
            >
              ...
              <span className="text-sm font-semibold text-gray-800">{userName ?? "Mon compte"}</span>
              <span className="text-xs text-gray-400 bg-brand-50 text-brand-600 px-1.5 py-0.5 rounded-full font-medium">Client</span>
            ...
            {dropdownOpen && (
              <div className="absolute right-0 top-11 w-52 bg-white rounded-xl shadow-lg border border-gray-100 py-1 z-50">
                <Link href="/dashboard" className="flex items-center gap-2.5 px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50" onClick={() => setDropdownOpen(false)}>
                  Mon tableau de bord
                </Link>
                <Link href="/dashboard/reservations" className="flex items-center gap-2.5 px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50" onClick={() => setDropdownOpen(false)}>
                  Mes réservations
                </Link>
                <Link href="/dashboard/messages" className="flex items-center gap-2.5 px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50" onClick={() => setDropdownOpen(false)}>
                  Mes messages
                </Link>
          ...
          <div className="flex items-center gap-2">
            <div className="flex items-center gap-2 bg-gray-50 border border-gray-200 rounded-xl px-3 py-1.5">
              ...
              <span className="text-sm font-semibold text-gray-800">{userName ?? "Mon compte"}</span>
              <span className="text-xs bg-brand-50 text-brand-600 px-1.5 py-0.5 rounded-full font-medium">Artisan</span>
            </div>
            <a href="https://artisan.manobra.fr/dashboard"
