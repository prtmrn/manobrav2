...
const navItems = [
  { href: "/dashboard/admin", label: "Vue d'ensemble", icon: "grid", exact: true },
  ...
  { href: "/dashboard/admin/litiges", label: "Litiges", icon: "flag" },
];
...
function Icon({ name }: { name: string }) {
  const icons: Record<string, React.ReactNode> = {
    ...
    calendar: <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.75} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>,
    chart: <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.75} d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" /></svg>,
    analytics: <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.75} d="M16 8v8m-4-5v5m-4-2v2m-2 4h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>,
    mail: <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.75} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /></svg>,
    ...
    flag: <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.75} d="M3 21v-4m0 0V5a2 2 0 012-2h6.5l1 1H21l-3 6 3 6h-8.5l-1-1H5a2 2 0 00-2 2zm9-13.5V9" /></svg>,
  };
...
export default function AdminShell({ children, userEmail }: { children: React.ReactNode; userEmail: string }) {
  ...
  return (
    <div className="min-h-screen bg-gray-950 flex">
      ...
      {/* ── SIDEBAR ── */}
      <aside className="w-56 flex-shrink-0 bg-gray-900 border-r border-gray-800 flex flex-col sticky top-0 h-screen">
        {/* Logo */}
        <div className="px-4 py-5 border-b border-gray-800">
          <Link href="/dashboard/admin" className="font-bold text-white text-lg tracking-tight">
            Man<span className="text-brand-400">obra</span>
            <span className="ml-2 text-[10px] font-semibold bg-brand-900 text-brand-300 px-1.5 py-0.5 rounded uppercase tracking-wider">Admin</span>
          </Link>
        </div>
        ...
        <div className="p-3 border-t border-gray-800">
          <div className="px-3 py-2 rounded-lg bg-gray-800 text-xs text-gray-400 truncate mb-2">
            {userEmail}
          </div>
      ...
      {/* ── CONTENU ── */}
      <main className="flex-1 overflow-auto bg-gray-950 text-white min-w-0">
        {children}
      </main>
    </div>
  );
